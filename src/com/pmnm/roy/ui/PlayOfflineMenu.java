...
public class PlayOfflineMenu extends DoaPanel {
	...
	TextImageButton newGameButton = DoaHandler.instantiate(TextImageButton.class, new DoaVectorF(1377f, 511f),
			UIInit.BUTTON_SIZE.x, UIInit.BUTTON_SIZE.y, DoaSprites.get(UIInit.BUTTON_IDLE_SPRITE),
			DoaSprites.get(UIInit.BUTTON_HOVER_SPRITE), "NEW_GAME", UIInit.FONT_COLOR, UIInit.HOVER_FONT_COLOR);
	TextImageButton loadGameButton = DoaHandler.instantiate(TextImageButton.class, new DoaVectorF(1377f, 584f),
			UIInit.BUTTON_SIZE.x, UIInit.BUTTON_SIZE.y, DoaSprites.get(UIInit.BUTTON_IDLE_SPRITE),
			DoaSprites.get(UIInit.BUTTON_HOVER_SPRITE), "LOAD_GAME", UIInit.FONT_COLOR, UIInit.HOVER_FONT_COLOR);
	...
	public PlayOfflineMenu(MainMenu mm, LoadMenu lm, ExitPopup ep) {
		...
		newGameButton.addAction(() -> {
			hide();
			mm.hide();
			spm = DoaHandler.instantiate(SinglePlayerMenu.class, this);
		});

		loadGameButton.addAction(new LoadButtonAction(mm, lm, ep, this));
