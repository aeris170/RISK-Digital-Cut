...
public class DifficultyComboButton extends DoaImageButton {
		...
		if (isEnabled && DoaMouse.MB1) {
			if (click) {
				...
				click = false;
					...
					&& DIFFICULTY_COMBO_BUTTONS.stream().allMatch(cb -> (!cb.click || (cb.click && cb.noneHit())))) {
				click = !click;
			}
		}
	}
	...
	private Rectangle2D hardHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.098f, position.y + Main.WINDOW_HEIGHT * 0.130f,
				Main.WINDOW_WIDTH * 0.115f, height * 0.8f);
	}

	private Rectangle2D insaneHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.098f, position.y + Main.WINDOW_HEIGHT * 0.158f,
				Main.WINDOW_WIDTH * 0.115f, height * 0.8f);
	}

	private Rectangle2D cheaterHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.098f, position.y + Main.WINDOW_HEIGHT * 0.186f,
				Main.WINDOW_WIDTH * 0.115f, height * 0.8f);
	}
