...
public class HostGameMenu extends DoaPanel {
	...
	public HostGameMenu() {
		...
		playButton.addAction(() -> {
			...
			Client.getInstance().sendToServer(new MessageBuilder().setSender("Client").setData(new GameInstance()).setType(MessageType.COMPRESSED).build());
		 ...
		 * DifficultyComboButton.DIFFICULTY_COMBO_BUTTONS.forEach(b ->
		 * DoaHandler.remove(b));
		 * DifficultyComboButton.DIFFICULTY_COMBO_BUTTONS.clear();
		 * DoaHandler.remove(playButton); DoaHandler.remove(backButton);
		 * DoaHandler.remove(prevMapButton); DoaHandler.remove(nextMapButton);
		 * ponm.show(); SocServer.stopServer(); }); */
