...
public class ColorComboButton extends DoaImageButton {
	...
	public void tick() {
		...
		if (isEnabled && DoaMouse.MB1) {
			if (click) {
				if (firstColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 0)) {
					index = 0;
				} else if (secondColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 1)) {
					index = 1;
				} else if (thirdColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 2)) {
					index = 2;
				} else if (fourthColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 3)) {
					index = 3;
				} else if (fifthColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 4)) {
					index = 4;
				} else if (sixthColorHitBox().contains(DoaMouse.X, DoaMouse.Y) && COMBO_BUTTONS.stream()
						.filter(cb -> cb != this && cb.isVisible).allMatch(cb -> cb.index != 5)) {
					index = 5;
				}
				click = false;
	...
	public void render(DoaGraphicsContext g) {
		if (!hidden) {
			...
			if (click) {
				...
				g.drawImage(DoaSprites.get("DropDownColor"), position.x - Main.WINDOW_WIDTH * 0.029f,
						position.y + Main.WINDOW_HEIGHT * 0.040f, Main.WINDOW_WIDTH * 0.050f, height);
				g.pushComposite();
				g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.4f));
				g.drawImage(DoaSprites.get("DropDownColorTex"), position.x - Main.WINDOW_WIDTH * 0.029f,
						position.y + Main.WINDOW_HEIGHT * 0.040f, Main.WINDOW_WIDTH * 0.050f, height);
				g.popComposite();
				for (int i = 0; i < OPTIONS.length; i++) {
					g.drawImage(OPTIONS[i], position.x - Main.WINDOW_WIDTH * 0.027f,
							position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * i),
	...
	private Rectangle2D firstColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 0),
				OPTIONS[0].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[0].getHeight());
	}

	private Rectangle2D secondColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 1),
				OPTIONS[1].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[1].getHeight());
	}

	private Rectangle2D thirdColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 2),
				OPTIONS[2].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[2].getHeight());
	}

	private Rectangle2D fourthColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 3),
				OPTIONS[3].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[3].getHeight());
	}

	private Rectangle2D fifthColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 4),
				OPTIONS[4].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[4].getHeight());
	}

	private Rectangle2D sixthColorHitBox() {
		return new Rectangle2D.Float(position.x - Main.WINDOW_WIDTH * 0.027f,
				position.y + Main.WINDOW_HEIGHT * 0.045f + (Main.WINDOW_HEIGHT * 0.040f * 5),
				OPTIONS[5].getWidth() + Main.WINDOW_WIDTH * 0.020f, OPTIONS[5].getHeight());
	}

	private boolean noneHit() {
		return !firstColorHitBox().contains(DoaMouse.X, DoaMouse.Y)
				&& !secondColorHitBox().contains(DoaMouse.X, DoaMouse.Y)
				&& !thirdColorHitBox().contains(DoaMouse.X, DoaMouse.Y)
				&& !fourthColorHitBox().contains(DoaMouse.X, DoaMouse.Y)
				&& !fifthColorHitBox().contains(DoaMouse.X, DoaMouse.Y)
